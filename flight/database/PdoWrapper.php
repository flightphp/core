...
class PdoWrapper extends PDO {
	 ...
	 * Example:
     * ```
     * $id = $db->fetchField("SELECT id FROM table WHERE something = ?", [$something]);
     * ```
	 * 
	 ...
	 * Example:
     * ```
     * $row = $db->fetchRow("SELECT * FROM table WHERE something = ?", [ $something ]);
     * ```
	 *
	 * @param string $sql   - Ex: `"SELECT * FROM table WHERE something = ?"`
	 * @param array<int|string,mixed> $params - Ex: `[$something]`
	 ...
	 * Example:
     * ```
     * $row = $db->fetchAll("SELECT * FROM table WHERE id = ? AND something IN(?), [ $id, [1,2,3] ]);
	 ...
	 */
	protected function processInStatementSql($sql, $params = []) {
